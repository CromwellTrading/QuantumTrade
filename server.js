...
bot.on('polling_error', (error) => {
    console.error('❌ Error de polling del bot:', error);
});
...
bot.onText(/\/start/, async (msg) => {
    ...
    console.log(`📨 Comando /start recibido de: ${userId} (@${username})`);
...
// Endpoint para información del bot
app.get('/bot-info', (req, res) => {
    res.json({
        bot_username: bot ? `@${bot.options.username}` : 'No inicializado',
        bot_status: bot ? '✅ Conectado' : '❌ Desconectado',
        supabase_status: supabase ? '✅ Conectado' : '❌ Desconectado',
        admin_id: ADMIN_ID,
        web_url: RENDER_URL
    });
});
...
app.listen(PORT, '0.0.0.0', () => {
    ...
    console.log(`🤖 Bot info: ${RENDER_URL}/bot-info`);
